...
class PrepareLogs:
    ...
    def choose_catylist_size(self, test_data, log):
        ...
        test_data['metcode'] = np.where((test_data['bp_eligible']) & (test_data['metcode_from_park'].isnull() == False) & (test_data['space_category'].isin(self.space_map[self.sector] + [''])), test_data['metcode_from_park'], test_data['metcode'])
        test_data['gsub'] = np.where((test_data['bp_eligible']) & (test_data['subid_from_park'].isnull() == False) & (test_data['space_category'].isin(self.space_map[self.sector] + [''])), test_data['subid_from_park'], test_data['gsub'])
        test_data['tot_size'] = np.where((test_data['bp_eligible']) & (test_data['tot_size_from_park'].isnull() == False) & (test_data['space_category'].isin(self.space_map[self.sector] + [''])), test_data['tot_size_from_park'], test_data['tot_size'])
        ...
        if self.sector == "ret":
            test_data['type1'] = np.where((test_data['bp_eligible']) & (test_data['type1_from_park'].isnull() == False) & (test_data['space_category'].isin(self.space_map[self.sector] + [''])) & (test_data['type1'] == ''), test_data['type1_from_park'], test_data['type1'])
            test_data['n_size'] = np.where((test_data['bp_eligible']) & (test_data['nonanchor_size_from_park'].isnull() == False) & (test_data['space_category'].isin(self.space_map[self.sector] + [''])), test_data['nonanchor_size_from_park'], test_data['n_size'])
            test_data['a_size'] = np.where((test_data['bp_eligible']) & (test_data['anchor_size_from_park'].isnull() == False) & (test_data['space_category'].isin(self.space_map[self.sector] + [''])), test_data['anchor_size_from_park'], test_data['a_size'])
        test_data['foundation_property_id'] = np.where((test_data['bp_eligible']) & (test_data['foundation_property_id_from_park'].isnull() == False) & (test_data['space_category'].isin(self.space_map[self.sector] + [''])), test_data['foundation_property_id_from_park'], test_data['foundation_property_id'])
    ...
    def append_nc_completions(self, combo, d_prop, log):
        ...
        temp_combo['in_log'] = 1
        temp_combo['property_source_id'] = temp_combo['property_source_id'].astype(str)
        nc_add = nc_add.join(temp_combo.drop_duplicates('property_source_id').set_index('property_source_id')[['in_log']], on='property_source_id')
        temp = nc_add.copy()
        temp = temp[(temp['in_log'].isnull() == False)]
        if len(temp) > 0:
            temp['reason'] = 'property is already in the log file'
        ...
        temp = nc_add.copy()
        temp = temp[((~temp['category'].isin(self.sector_map[self.sector]['category'])) | (~temp['subcategory'].isin(self.sector_map[self.sector]['subcategory'] + ['']))) & (nc_add['mixed_use_check'] == False)]
        if len(temp) > 0:
        ...
        if self.sector == "ind":
            ...
            if len(temp) > 0:
                temp['reason'] = 'property is warehouse office and there is no size by use data to guide subsector assignment'
                temp['value'] = 'warehouse_office'
                self.drop_nc_log = self.drop_nc_log.append(temp.drop_duplicates('property_source_id')[['property_source_id', 'reason', 'value']])
        ...
        nc_add['size'] = np.where((nc_add['buildings_size_rentable_sf'] > 100), nc_add['buildings_size_rentable_sf'], nc_add['size'])
        nc_add['size'] = np.where((nc_add[size_by_use] > 0) & (nc_add[size_by_use] <= nc_add['size']) & ((nc_add['off_perc'] < 0.25) | (nc_add['off_perc'].isnull() == True) | (~nc_add['subcategory'].isin(['warehouse_flex', 'warehouse_office']))), nc_add[size_by_use], nc_add['size'])
        if self.sector in ['off', 'ind']:
        ...
        temp = nc_add.copy()
        temp = temp[(temp['size'].isnull() == True) | ((temp['size'] < 10000) & (self.sector in ['off', 'ind']))]
        if len(temp) > 0:
            temp['reason'] = 'building less than 10k sqft'
        ...
        temp = nc_add.copy()
        temp = temp[(temp['property_geo_msa_code'] == '') | (temp['property_geo_subid'].isnull() == True)]
        if len(temp) > 0:
                    ...
                    if er_id[0] == self.sector_map[self.sector]['prefix']:
                        ...
                        if er_id[1:] in list(log_dict.keys()):
                            ...
                            if test1 and test2 and (test3 or test4) and (test5 or test6) and test7:
                                if num == row['location_street_address'].strip().split(' ').str[0] and len(num) > 2:
                                    building_test = False
                                else:
                                    building_test = True
                            else:
        ...
        temp = nc_add.copy()
        temp = temp[(temp['month'].isnull() == True)]
        if len(temp) > 0:
            temp['reason'] = 'missing month built'
            self.drop_nc_log = self.drop_nc_log.append(temp.drop_duplicates('property_source_id')[['property_source_id', 'reason']])
